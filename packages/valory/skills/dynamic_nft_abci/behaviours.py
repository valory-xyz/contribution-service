    ...
    def update_layers(self):  # pragma: no cover
        ...
        for layer_name in self.ImageManager.LAYER_NAMES:
            ...
            if api_layer_hashes != local_layer_hashes:
                self.context.logger.info(
                    f"Layer '{layer_name!r}' is out of date. Local={local_layer_hashes}, API={api_layer_hashes} Re-downloading."
                )
                # Remove local images
    ...
    class ImageManager:
        ...
        def generate(self, image_code: str) -> Optional[Image.Image]:
            ...
            if len(image_code) != self.CODE_LEN:
                self.logger.error(
                    f"ImageManager: invalid code '{image_code!r}'. Length is {len(image_code)}, should be {self.CODE_LEN}."
                )
                return None
            ...
            for layer_index, layer_code in enumerate(img_layer_codes):
                if layer_code >= len(self.layers[layer_index]):
                    self.logger.error(
                        f"ImageManager: invalid code '{image_code!r}'. Layer {layer_index} code must be lower than {len(self.layers[layer_index])}. Found {layer_code}."
                    )
                    return None
