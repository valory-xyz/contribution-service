    ...
    def get_hash_by_package_line(
        ...
        try:
            ...
            if not m_command and not m_package:
                print(
                    f"[{target_file}]: line '{package_line}' does not match an autonomy/aea command or package format"  # noqa: B028
                )
                return None
            ...
            if not d["vendor"] and not d["package"]:
                ...
                print(
                    f"[{target_file}]: unknown IPFS hash in line '{package_line}'. Can't fix because this command just uses the hash"  # noqa: B028
                )
                return None
        ...
        # Otherwise log the error
            ...
            print(
                f"[{target_file}]: could not find the corresponding hash for line '{package_line}'"  # noqa: B028
            )
            return None
